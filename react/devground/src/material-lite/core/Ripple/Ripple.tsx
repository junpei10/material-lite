...
class MatRipple extends Component<MatRippleProps> {
  ...
  /** @lifecycle */
  componentDidMount() {
    this.ref.current.parentElement.classList.add('Ml-ripple');
  ...
  componentDidUpdate(prevProps: MatRippleProps) {
    if (this.props.disabled) {
      ...
      if (listenerTargetProp !== prevProps.target || this.prevDisabled) {
        // Remove listener
        this.pointerdownHandler();
        ...
        const containerElement =
          (this.props.wrapped) ? refElement : hostElement;
  ...
  fadeInRipple(
    ...
    (this.props.theme) ?
      ...
      rippleStyles += `background-color: ${this.props.color || 'currentColor'};`;
  ...
  render() {
    return <div className={this.props.wrapped ? 'Ml-ripple-outlet' : 'Ml-ref'} ref={this.ref}></div>
  }
}
